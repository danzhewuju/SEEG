...
class miniImageNetBuilder:
    ...
    def run_validation_epoch(self):
        ...
        total_data = {"accuracy": [], "precision": [], "recall": [], "f1": [], "auc": []}
        for batch_idx, (x_support_set, y_support_set, x_target, target_y) in pbar:
            ...
            if self.isCudaAvailable:
                acc, c_loss_value, performance = self.matchingNet(x_support_set.cuda(), y_support_set_one_hot.cuda(),
                                                                  x_target.cuda(), y_target.cuda())
            else:
                acc, c_loss_value, performance = self.matchingNet(x_support_set, y_support_set_one_hot,
                                                                  x_target, y_target)
            for key in performance.keys():
                total_data[key].append(performance[key])
        ...
        return total_val_c_loss, total_val_accuracy, total_data
    ...
    def run_testing_epoch(self):
        ...
        for batch_idx, (x_support_set, y_support_set, x_target, target_y) in pbar:
            ...
            if self.isCudaAvailable:
                acc, c_loss_value, json_data = self.matchingNet(x_support_set.cuda(), y_support_set_one_hot.cuda(),
                                                                x_target.cuda(), y_target.cuda())
            else:
                acc, c_loss_value, json_data = self.matchingNet(x_support_set, y_support_set_one_hot,
                                                                x_target, y_target)
    ...
    def save_model(self):
        save_path = os.path.join("./models", "matching.pkl")
        torch.save(self.matchingNet.state_dict(), save_path)
        print("Matching model has been saved!")
