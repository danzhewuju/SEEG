...
def vae_data(raw_path, save_path):  # train-test dataset
...
def vae_data_val(raw_path, save_path):
    names = os.listdir(raw_path)
    ...
    for n in names:
        ...
        vae_model_p.load_state_dict(torch.load(model_path_all))
        vae_model_p.eval()
        ...
        for p in file_names:
            ...
            result = result.astype('float32')
            result = result[np.newaxis, :]
            # result = trans_data(vae_model_n, result)
            result = trans_data(vae_model_p, result)
            save_numpy_info(result, save_data_path)
        print("all files:{} had been saved!".format(len(file_names)))
...
if __name__ == '__main__':
    ...
    # vae_data(train_path, save_train_dir)
    # vae_data(test_path, save_test_dir)
    vae_data_val(val_path, save_val_dir)
